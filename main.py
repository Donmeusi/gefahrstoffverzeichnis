...
# Standardtexte der Betriebsanweisung, wenn nichts individuell gespeichert wurde.
# Diese Konstanten sind die *einzige* Quelle: sowohl die Druckansicht
# (templates/ba_print.html) als auch die PDF-Ablage unten greifen darauf zu.
# Stünden sie an zwei Stellen, könnten gedruckte Betriebsanweisung und abgelegtes
# PDF unterschiedlichen Wortlaut zeigen.
BA_STANDARD_TEXTE = {
    'ba_taetigkeit': 'Umgang mit Gefahrstoffen',
    'ba_verhalten_verschuetten': (
        '<ul>'
        '<li>Gefahrenbereich räumen und absperren.</li>'
        '<li>Verschüttete Mengen mit geeignetem Bindemittel aufnehmen.</li>'
        '</ul>'
    ),
    'ba_verhalten_brand': (
        '<ul>'
        '<li>Brände in der Umgebung mit geeignetem Löschmittel (siehe Brandschutzordnung) bekämpfen.</li>'
        '<li>Gefäß wenn möglich aus der Gefahrenzone entfernen oder mit Wasser kühlen.</li>'
        '</ul>'
    ),
    'ba_erste_hilfe_massnahme': (
        '<ul>'
        '<li>Auf Selbstschutz achten, ärztliche Behandlung. Lebensrettende Sofortmaßnahmen, wie '
        '\'Stabile Seitenlage\', \'Herz-Lungen-Wiederbelebung\', \'Schockbekämpfung\' müssen '
        'situationsabhängig durchgeführt werden. Wunden keimfrei bedecken. Für Körperruhe sorgen, '
        'vor Wärmeverlust schützen. Ersthelfer hinzuziehen.</li>'
        '</ul>'
    ),
    'ba_erste_hilfe_kontakt': (
        '<ul>'
        '<li>Nach Einatmen: Frischluftzufuhr, bei Beschwerden Arzt aufsuchen.</li>'
        '<li>Nach Hautkontakt: Sofort mit viel Wasser und Seife abwaschen.</li>'
        '<li>Nach Augenkontakt: Augen mehrere Minuten bei geöffnetem Lidspalt unter fließendem '
        'Wasser spülen.</li>'
        '<li>Nach Verschlucken: Mund ausspülen und reichlich Wasser nachtrinken. Kein Erbrechen '
        'herbeiführen.</li>'
        '</ul>'
    ),
    'ba_entsorgung': (
        'Nicht in Ausguss oder Mülltonne schütten. Größere Mengen in geeigneter Flasche sammeln '
        'und nach Absprache mit dem Entsorgungsbeauftragten im Sonderabfall-Lager lagern.'
    ),
}


def ba_text_oder_standard(ba_texte, feld):
    """Liefert den gespeicherten Text oder den Standardtext."""
    gespeichert = (ba_texte or {}).get(feld)
    return gespeichert if gespeichert else BA_STANDARD_TEXTE.get(feld, '')
...
def save_bytes_with_stoff_name(daten, stoff_name, suffix="", ext="pdf"):
    """Wie save_file_with_stoff_name, aber für bereits erzeugte Daten (z. B. ein PDF)."""
    upload_dir = app.config.get('UPLOAD_FOLDER', os.path.join(basedir, 'data', 'uploads'))
    os.makedirs(upload_dir, exist_ok=True)

    safe_name = re.sub(r'[^a-zA-Z0-9_\-\.]', '_', stoff_name or '')
    if not safe_name.strip('_'):
        safe_name = "stoff"
    base_name = f"{safe_name}_{suffix}" if suffix else safe_name
    ...
    filename = secure_filename(f"{base_name}.{ext}")
    if not filename:
        filename = f"dokument_{suffix}.{ext}"
...
#
# Beim Speichern des Entwurfs wird die Betriebsanweisung zusätzlich als PDF
# abgelegt, damit sie am Gefahrstoff hängt wie ein hochgeladenes Dokument
# (Dokumentenliste, Download). ACHTUNG: das Layout ist hier ein zweites Mal
# umgesetzt - die Druckansicht ist HTML. Wird das Druckbild geändert, muss die
# Erzeugung hier nachgezogen werden. Die Texte dagegen kommen aus derselben
# Quelle (BA_STANDARD_TEXTE bzw. ba_texte), damit gedruckte Betriebsanweisung
# und abgelegtes PDF nicht auseinanderlaufen.


class _BATextAusHtml(HTMLParser):
    """Wandelt gespeichertes BA-HTML in reinen Text mit Zeilenumbrüchen (für das PDF)."""
...
def ba_html_zu_text(html):
    """BA-HTML als reiner Text, ein Listeneintrag bzw. Absatz je Zeile."""
    if not html:
        return ''
    parser = _BATextAusHtml()
    try:
        parser.feed(html)
        parser.close()
        return parser.text()
...
def ba_kontext(stoff):
    """Arbeitsbereich sowie H- und P-Sätze mit Text - von Druckansicht und PDF genutzt."""
    ...
    for h in (stoff.h_saetze or "").split(","):
        code = h.strip()
        if code:
            text = h_dict.get(code, "")
            h_liste.append(f"{code}: {text}" if text else code)

    p_liste = []
    for p in (stoff.p_saetze or "").split(","):
        code = p.strip()
        if code:
            text = p_dict.get(code, "")
            p_liste.append(f"{code}: {text}" if text else code)

    return arbeitsbereich, h_liste, p_liste


def ba_pdf_erzeugen(stoff, arbeitsbereich, h_saetze_liste, p_saetze_liste):
    """Erzeugt die Betriebsanweisung als PDF und liefert ein BytesIO."""
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.styles import ParagraphStyle
    from reportlab.lib.units import mm
    from reportlab.platypus import (SimpleDocTemplate, Table, TableStyle, Paragraph,
                                    Spacer, Image as PdfBild)
    ...
    breite = A4[0] - 2 * RAND
    icon_spalte = 26 * mm
    kopf_rechts_breite = breite * 0.25 - 10

    texte = load_ba_texte(stoff)
    gewaehlte_zeichen = load_ba_gebotszeichen(stoff)

    s_text = ParagraphStyle('ba_text', fontName='Helvetica', fontSize=9.5, leading=11.5)
    s_klein = ParagraphStyle('ba_klein', parent=s_text, fontSize=8, leading=10)
    s_fett = ParagraphStyle('ba_fett', parent=s_text, fontName='Helvetica-Bold')
    s_kursiv = ParagraphStyle('ba_kursiv', parent=s_text, fontName='Helvetica-Oblique')
    s_titel = ParagraphStyle('ba_titel', fontName='Helvetica-Bold', fontSize=16, leading=19,
                             alignment=1)
    s_name = ParagraphStyle('ba_name', parent=s_titel, fontSize=18, leading=21)
    s_balken = ParagraphStyle('ba_balken', fontName='Helvetica-Bold', fontSize=10.5, leading=13,
                              textColor=colors.white)
    s_balken_rechts = ParagraphStyle('ba_balken_rechts', parent=s_balken, alignment=2)

    def esc(text):
        return (text or '').replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')

    def absatz(text, stil):
        return Paragraph(esc(text).replace('\n', '<br/>'), stil)

    def balken(titel, rechts=None):
        if rechts:
            daten = [[absatz(titel, s_balken), absatz(rechts, s_balken_rechts)]]
            spalten = [breite * 0.62, breite * 0.38]
        else:
            daten = [[absatz(titel, s_balken)]]
            spalten = [breite]
        t = Table(daten, colWidths=spalten)
    ...
    def zeile(icons, inhalt, unten_linie=True):
        t = Table([[icons, inhalt]], colWidths=[icon_spalte, breite - icon_spalte])
        stil = [
            ...
            ('LINEAFTER', (0, 0), (0, 0), 0.5, ROT),
        ]
        if unten_linie:
            stil.append(('LINEBELOW', (0, 0), (-1, -1), 0.5, ROT))
        t.setStyle(TableStyle(stil))
        return t

    def svg_flowable(relpfad, groesse):
        vollpfad = os.path.join(basedir, relpfad)
        if not os.path.exists(vollpfad):
            return None
        try:
            d = svg2rlg(vollpfad)
        except Exception as e:
            print(f"SVG '{relpfad}' konnte nicht gelesen werden: {e}")
            return None
        if d is None or not d.width or not d.height:
            return None
        urspruenglich_breit, urspruenglich_hoch = d.width, d.height
        faktor = groesse / max(urspruenglich_breit, urspruenglich_hoch)
        d.scale(faktor, faktor)
        # Wichtig: Drawing.scale() setzt nur die Transform, nicht width/height.
        # Ohne diese zwei Zeilen fordert das Symbol im Layout seine volle
        # Originalgröße an (nachgemessen: weiter 434 statt 31 Punkte) und
        # quetscht die Textspalte daneben auf einen Streifen.
        d.width = urspruenglich_breit * faktor
        d.height = urspruenglich_hoch * faktor
        d.hAlign = 'CENTER'
        return d

    def icon_stapel(relpfade, groesse=9 * mm):
        teile = [x for x in (svg_flowable(r, groesse) for r in relpfade) if x is not None]
        return teile or [absatz('-', s_text)]

    def unterschrift_flowable():
        u = load_ba_unterschrift(stoff)
        if u.get('typ') == 'bild':
            try:
                roh = base64.b64decode(u['wert'].split(',', 1)[1])
                bild = PdfBild(BytesIO(roh))
                faktor = min(kopf_rechts_breite / bild.imageWidth,
                             (9 * mm) / bild.imageHeight)
                bild.drawWidth = bild.imageWidth * faktor
                bild.drawHeight = bild.imageHeight * faktor
                bild.hAlign = 'CENTER'
                return bild
            except Exception as e:
                print(f"Unterschrift konnte nicht eingebettet werden: {e}")
        elif u.get('typ') == 'name':
            return absatz('gez. ' + u['wert'], s_kursiv)
        leeres_feld = Table([['']], colWidths=[kopf_rechts_breite], rowHeights=[9 * mm])
        leeres_feld.setStyle(TableStyle([('BOX', (0, 0), (-1, -1), 0.5, ROT)]))
        return leeres_feld

    def rahmen(canvas, doc):
        canvas.saveState()
        canvas.setStrokeColor(ROT)
        canvas.setLineWidth(1.4)
        canvas.rect(RAND, RAND, breite, A4[1] - 2 * RAND)
        canvas.restoreState()

    # Kopf
    nummer = ba_html_zu_text(texte.get('ba_nummer', ''))
    taetigkeit = ba_html_zu_text(ba_text_oder_standard(texte, 'ba_taetigkeit'))
    kopf = Table(
        [[[absatz('Arbeitsbereich:', s_fett), Spacer(1, 3), absatz(arbeitsbereich, s_text)],
          [absatz('Betriebsanweisung', s_titel), absatz('gemäß §14 GefStoffV', s_fett)],
          [absatz('Nummer: ' + nummer, s_text), absatz('(Prüfung jährlich)', s_fett),
           absatz('Unterschrift:', s_fett), unterschrift_flowable()]],
         [absatz('Tätigkeit: ' + taetigkeit, s_text), '', '']],
        colWidths=[breite * 0.25, breite * 0.5, breite * 0.25])
    kopf.setStyle(TableStyle([
        ('SPAN', (0, 1), (2, 1)),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
        ('BOX', (0, 0), (-1, -1), 0.5, ROT),
        ('INNERGRID', (0, 0), (-1, 0), 0.5, ROT),
    ...
    bezeichnung = [absatz('Umgang / Tätigkeit mit', s_text), absatz(stoff.name, s_name)]
    if stoff.cas_nummer:
        bezeichnung.append(absatz('CAS: ' + stoff.cas_nummer, s_klein))
    bezeichnung_tabelle = Table([[bezeichnung]], colWidths=[breite])
    bezeichnung_tabelle.setStyle(TableStyle([
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
        ('TOPPADDING', (0, 0), (-1, -1), 6),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 6),
        ('LINEBELOW', (0, 0), (-1, -1), 0.5, ROT),
    ]))

    # Abschnittsinhalte
    ghs_pfade = ['static/pictograms/%s.svg' % c.strip()
                 for c in (stoff.piktogramme or '').split(',') if c.strip()]
    zeichen_pfade = ['static/symbols/%s.svg' % c for c in gewaehlte_zeichen]

    h_text = (ba_html_zu_text(texte.get('ba_h_saetze', ''))
              or '\n'.join(h_saetze_liste) or 'Keine H-Sätze hinterlegt.')
    p_text = (ba_html_zu_text(texte.get('ba_schutzmassnahmen', ''))
              or '\n'.join(p_saetze_liste)
              or 'Keine P-Sätze hinterlegt. Bitte spezifische Schutzmaßnahmen beachten.')

    schutzmassnahmen = [
        absatz('Technische und Organisatorische / Persönliche / Hygienische Schutzmaßnahmen:', s_fett),
        absatz(p_text, s_text),
    ]
    verhalten = [
        absatz('Verschütten:', s_fett),
        absatz(ba_html_zu_text(ba_text_oder_standard(texte, 'ba_verhalten_verschuetten')), s_text),
        Spacer(1, 2),
        absatz('Brand:', s_fett),
        absatz(ba_html_zu_text(ba_text_oder_standard(texte, 'ba_verhalten_brand')), s_text),
    ]
    erste_hilfe = [
        absatz('Bei jeder Erste-Hilfe-Maßnahme:', s_fett),
        absatz(ba_html_zu_text(ba_text_oder_standard(texte, 'ba_erste_hilfe_massnahme')), s_text),
        Spacer(1, 2),
        absatz('Einatmen / Hautkontakt / Augenkontakt / Verschlucken:', s_fett),
        absatz(ba_html_zu_text(ba_text_oder_standard(texte, 'ba_erste_hilfe_kontakt')), s_text),
    ]
    entsorgung = [absatz(ba_html_zu_text(ba_text_oder_standard(texte, 'ba_entsorgung')), s_text)]
    entsorgung_tabelle = Table([[entsorgung]], colWidths=[breite])
    entsorgung_tabelle.setStyle(TableStyle([
        ('LEFTPADDING', (0, 0), (-1, -1), 3),
        ('RIGHTPADDING', (0, 0), (-1, -1), 3),
        ('TOPPADDING', (0, 0), (-1, -1), 4),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 4),
    ...
    story = [
        kopf,
        balken('GEFAHRSTOFFBEZEICHNUNG'), bezeichnung_tabelle,
        balken('GEFAHREN FÜR MENSCH UND UMWELT', 'Signalwort: ' + (stoff.signalwort or '-')),
        zeile(icon_stapel(ghs_pfade, 11 * mm), absatz(h_text, s_text)),
        balken('SCHUTZMASSNAHMEN UND VERHALTENSREGELN'),
        zeile(icon_stapel(zeichen_pfade, 9 * mm), schutzmassnahmen),
        balken('VERHALTEN IM GEFAHRFALL', 'Auf Selbstschutz achten! (0) – 112'),
        zeile(icon_stapel(['static/symbols/F001.svg'], 11 * mm), verhalten),
        balken('ERSTE HILFE', 'Ersthelfer: siehe Aushang (0) – 112'),
        zeile(icon_stapel(['static/symbols/E003.svg'], 11 * mm), erste_hilfe),
        balken('SACHGERECHTE ENTSORGUNG'), entsorgung_tabelle,
    ]

    puffer = BytesIO()
    doc = SimpleDocTemplate(puffer, pagesize=A4,
                            leftMargin=RAND, rightMargin=RAND,
                            topMargin=RAND, bottomMargin=RAND,
                            title=f'Betriebsanweisung {stoff.name}',
                            author='Gefahrstoffverzeichnis')
    doc.build(story, onFirstPage=rahmen, onLaterPages=rahmen)
...
def betriebsanweisung_speichern(id):
    ...
    try:
        ...
        # Den Entwurf zusätzlich als Betriebsanweisung am Gefahrstoff ablegen,
        # damit er dort hängt wie ein hochgeladenes Dokument. Bewusst nur, wenn
        # noch keine hinterlegt ist: ein hochgeladenes, unterschriebenes PDF soll
        # nicht versehentlich durch den Entwurf ersetzt werden.
        if stoff.betriebsanweisung:
            flash(f'Am Gefahrstoff ist bereits eine Betriebsanweisung hinterlegt '
                  f'({stoff.betriebsanweisung}) — sie wurde nicht ersetzt.', 'info')
        else:
            try:
                arbeitsbereich, h_liste, p_liste = ba_kontext(stoff)
                pdf = ba_pdf_erzeugen(stoff, arbeitsbereich, h_liste, p_liste)
                dateiname = save_bytes_with_stoff_name(pdf, stoff.name, "BA", "pdf")
                stoff.betriebsanweisung = dateiname
                db.session.commit()
                log_audit_event('UPDATE', 'Gefahrstoff', id,
                                f'Betriebsanweisung als {dateiname} abgelegt.')
                flash(f'Betriebsanweisung als {dateiname} am Gefahrstoff abgelegt.', 'success')
            except Exception as pdf_fehler:
                db.session.rollback()
                print(f"Betriebsanweisung konnte nicht als PDF abgelegt werden: {pdf_fehler}")
                flash('Der Entwurf wurde gespeichert, konnte aber nicht als PDF '
                      'abgelegt werden.', 'error')
    except Exception as e:
        db.session.rollback()
