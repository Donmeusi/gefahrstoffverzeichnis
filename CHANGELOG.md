...
* **Auswahlfenster begrenzt (auf 4)**: Die Icon-Spalte ist 100 px breit. Die Grenze wurde **empirisch** ermittelt, nicht geschätzt: mit vier Zeichen passt die Betriebsanweisung bei mittellanger P-Satz-Liste noch auf eine A4-Seite, mit fünf läuft sie über. Dazu ein Zähler im Auswahlfenster und die Sperre der noch freien Kästchen. Die Zeichen werden 38 px groß in zwei Spalten umgebrochen. Bei sehr langen Texten kann die BA unabhängig davon umbrechen — die Grenze schützt die Spalte, nicht das ganze Dokument.
...
* **Erzwungene Seitenumbruch-Regel entfernt**: `.ba-container { page-break-after: always }` hat nach jedem Druck eine leere Seite angehängt. Nachgemessen: mit der Regel zwei Seiten, ohne sie eine.
* **⚠️ Offen — leere Seite durch `@page margin: 0` + 10 mm Padding**: Der Inhalt plus Padding ist rund 10 px höher als die Seitenbox, deshalb hängt Chrome eine leere zweite Seite an. Nachgemessen unter `@page margin: 0`: Padding ≤ 6 mm ergibt eine saubere Seite, 8–10 mm ergeben zwei. Aktuell stehen 10 mm. Eine Änderung auf 6 mm behebt es, lässt aber nur ~1 mm Abstand zur nicht bedruckbaren Kante vieler Drucker. Alternative: `@page { margin: 10mm }` und das Padding entfernen — robust, dafür können die Kopf-/Fußzeilen des Browsers wieder erscheinen. **Diese Entscheidung ist bewusst offengelassen.**
* **Druck-Hinweis (unverändert)**: Das Padding liegt auf `body` und gilt einmal pro Dokument, nicht pro Seite — bei einer mehrseitigen Betriebsanweisung hätte Seite 2 keinen Rand.
